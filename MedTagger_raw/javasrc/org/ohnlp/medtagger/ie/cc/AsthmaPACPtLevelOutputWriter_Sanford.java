 ...
 */
public class AsthmaPACPtLevelOutputWriter_Sanford extends CasConsumer_ImplBase {
	class Info {
		String date;
		String docName;
		String concept;
	...
	public void initialize() throws ResourceInitializationException {
		...
		diagSections.add("IMPRESSION");
		diagSections.add("IMPRESSIONS");
		diagSections.add("CLINICAL IMPRESSION");
		diagSections.add("IMPRESSION AND DISCUSSION");

		exclSections = new HashSet<String>();
		exclSections.add("FAMILY HISTORY"); 
		exclSections.add("FAMILY MEDICAL HISTORY"); 
		exclSections.add("SOCIAL HISTORY"); 
		exclSections.add("SOCIAL"); 
		exclSections.add("SOCIAL SCREENING"); 
		exclSections.add("GENERAL PREOP INSTRUCTIONS"); 
		
		exclNotes = new HashSet<String>();
		exclNotes.add("Patient-Instructions"); 
		exclNotes.add("Operative-Note"); 
		exclNotes.add("Anesthesia-Post-procedure-Evaluation"); 
		exclNotes.add("Anesthesia-Transfer-of-Care"); 
		exclNotes.add("Anesthesia-Pre-procedure-Evaluation"); 
		exclNotes.add("Pre-Anesthesia"); 
		exclNotes.add("Post-Anesthesia"); 
		exclNotes.add("PostOp-Progress-Note"); 
		exclNotes.add("OR-PreOp"); 
		exclNotes.add("OR-PostOp");
		exclNotes.add("OR-Nursing"); 
		exclNotes.add("Occupational-Therapy"); 
		exclNotes.add("Speech-Therapy"); 
		exclNotes.add("Psychology"); 
		exclNotes.add("CCHS-Autism-Screening");
		exclNotes.add("Care-Conference"); 
		...
		try {
			...
			String line="";
			//clinic\tlastfudate\tstatus\tinxdate
			while((line=br.readLine()) != null) {
				if(line.startsWith("//")) continue;
				String[] str = line.split("\\t");
				lastFuDate.put(String.format("%07d", Integer.parseInt(str[0])), str[1]);
			}	
			br.close();
	...
	public void processCas(CAS aCAS) throws ResourceProcessException {
		...
		if (docIterator.hasNext()) {
			...
			String[] docf =  docName.split("_");

			//System.out.println("---"+docName+" processing---");
			//docName: E2678091_CLINICALNOTE_Care-Planning_2011-11-01T08-16-00.txt
			mcn = docf[0].substring(1); //7 digits (excluded E)
			if(docf.length!=4) {
				System.out.println("Incorrect docname:"+docName);
				System.exit(1);
			}
			noteType = docf[2];
			noteDate = docf[3].split("T")[0].replaceAll("-", ""); //yyyymmdd
		}
		...
		if(exclNotes.contains(noteType)) skip = true;
