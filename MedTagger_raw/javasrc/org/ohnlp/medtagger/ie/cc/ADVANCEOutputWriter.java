 ...
 *
 */
public class ADVANCEOutputWriter extends CasConsumer_ImplBase {
	class Info {
		String date;
		String docName;
		String concept;
			...
			for(String date : dateInfoMap.keySet()) {				
				...
				if(isabs_method && (isabs_abstractor || isabs_participant || isabs_data || isabs_EHR)) {
					for (Integer i : sentIntMethod) {
						...
						if (sentIntParticipant.contains(i) && (sentIntAbstractor.contains(i) || sentIntData.contains(i) || sentIntEHR.contains(i) ||sentIntSample.contains(i) )){
							sentout += sentIntMethodTxt.get(i).replace("<abs_method>", "<abs_pro_method>");
							is_probable_abs = true;
						}
						if (sentIntTraining.contains(i)){
							sentout += sentIntMethodTxt.get(i).replace("<abs_method>", "<abs_training>");
							training = "Yes";
						}
						if (sentIntValidation.contains(i)){
							sentout += sentIntMethodTxt.get(i).replace("<abs_method>", "<abs_validation>");
							validation = "Yes";
						}
						if (sentIntSample.contains(i)){
							sentout += sentIntMethodTxt.get(i).replace("<abs_method>", "<abs_sample>");
							sample = "Yes";
						}
				...
				//priority
				String fxt = "";
				if(is_definite_abs) fxt = "DEFINITE_ABS";
				else if(is_probable_abs) fxt = "PROBABLE_ABS";
				else {
